    ...
    if mtext == '冥想引導':
        try:
            messages = [
                ...
                TextSendMessage(text='請輕輕閉上眼睛，把注意力帶回呼吸。每一次吸氣，想像自己吸入清新與平靜；每一次吐氣，釋放緊張與雜念。你不需要改變什麼，只要觀察——觀察自己的身體，觀察心中的波動。當念頭來臨，不要抗拒，也不需追隨，就讓它們像雲一樣飄過。你在這裡，這一刻就好，安住於呼吸，安住於當下。')
            ]
            line_bot_api.reply_message(event.reply_token, messages)
        except Exception as e:
            print("錯誤內容：", e)
